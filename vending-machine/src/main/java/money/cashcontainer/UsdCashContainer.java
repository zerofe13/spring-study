...
public class UsdCashContainer implements CashContainer{
    private static Map<Integer,Integer> coinQuantity = new HashMap<Integer,Integer>(){
        {
            Arrays.stream(MoneyInfo.usdIntegerCoinList).forEachOrdered(i -> put(i, 0));
        }
    };
    private static Map<Integer,Integer> billQuantity = new HashMap<Integer,Integer>(){
        {
            Arrays.stream(MoneyInfo.usdIntegerBillList).forEachOrdered(i -> put(i, 0));
        }
    };
